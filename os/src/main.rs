...
pub extern "C" fn rust_main(_hart_id: usize, dtb_pa: PhysicalAddress) -> ! {
    ...
    kernel_thread(sample as usize, Some(&[0usize]));
    //user_thread("notebook");
    user_thread("hello_world");
    PROCESSOR.get().run()
}
...
fn kernel_thread(entry_point: usize, arguments: Option<&[usize]>) {
    let process = Process::new_kernel().unwrap();
    let thread = Thread::new(process, entry_point, arguments).unwrap();
    PROCESSOR.get().add_thread(thread);
}

fn user_thread(name: &str) {
    // 从文件系统中找到程序
